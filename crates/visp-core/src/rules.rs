...
impl RuleEngine {
    pub fn new(project_path: &Path) -> std::io::Result<Self> {
        ...
        if let Some(home) = home_dir() {
            let global_agents = home.join(".config").join("visp").join("AGENTS.md");
        ...
        let project_rules = project_path.join(".visp").join("rules");
        ...
        if let Some(home) = home_dir() {
            let global_rules = home.join(".config").join("visp").join("rules");
