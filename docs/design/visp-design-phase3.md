# visp Phase 3 阶段设计：Agent 核心 + Daemon
...
Phase 3 涉及一个已有 crate 的扩展（visp-core），一个已有 crate 的协议升级（visp-proto），和一个新 crate：

| Crate | 职责 | 类型 |
|---|---|---|
| **visp-core** | 新增：Rule Engine、Session Manager、Prompt Builder、Tool Registry、Agent 编排循环 | 扩展 |
| **visp-proto** | 新增：UserQuery / UserResponse 消息类型，支持 Human-in-Loop | 扩展 |
| **visp-daemon** | gRPC server、配置加载、CoderDaemon Service 实现 | 新建 |

### 2.1 visp-core 扩展

visp-core 从纯类型定义扩展为承载核心业务逻辑。新增 5 个模块。
...
1. 项目级：`.visp/system-prompt.md`
2. 用户全局：`~/.config/visp/system-prompt.md`
3. 内置默认：visp-core 硬编码的 visp 角色定义
...
**AgentErrorCode 枚举**（定义于 visp-core，`AgentEvent::Error` 携带）：
...
**配置项**（AgentConfig 结构体，定义于 visp-core）：
...
**依赖**：visp-core, visp-proto, visp-llm, visp-tools, tonic, tokio, toml, tracing, tracing-subscriber
...
2. 环境变量（`VBW_` 前缀）
3. 用户配置（`~/.config/visp/daemon.toml`）
4. 内置默认值
...
**职责**：实现 proto 定义的 `CoderDaemon` trait，将 gRPC 请求翻译为 visp-core 模块调用。
...
visp-daemon (二进制)
    │
    ├──→ visp-proto   (gRPC 类型 + service trait)
    ├──→ visp-core    (Agent, Session, Rules, Prompt, ToolRegistry)
    ├──→ visp-llm     (LlmProvider 实现)
    └──→ visp-tools   (Tool 实现)
    
visp-core (扩展)
    │ 新增依赖: tokio, notify
    │
    ├──→ visp-llm     (通过 LlmProvider trait，编译时依赖)
    └──→ visp-tools   (通过 Tool trait，编译时依赖)

注意：visp-core 不直接依赖 visp-llm 或 visp-tools crate，
...
- `cargo build --workspace` 编译通过（包含新 crate visp-daemon）
- `cargo test --workspace` 所有测试通过
- `cargo clippy --workspace -- -D warnings` 通过
- `cargo fmt --check --all` 通过
- visp-core 新增模块的单元测试覆盖（每个模块至少 3 个测试用例）
