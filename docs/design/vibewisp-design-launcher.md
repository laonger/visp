...
```
1. 解析 CLI 参数
2. 创建 ~/.vibewisp/logs/ 目录
3. 启动 vbw-daemon 子进程（默认地址 [::1]:50051），stdout/stderr 写入日志文件
4. 轮询 health check（最多 30 次，间隔 500ms，共 15s 超时）
5. 超时 → 打印错误 → kill daemon → 退出码 1
6. 启动 vbw-cli 子进程，传递 CLI 参数，CLI 的 stdout/stderr 直通终端
7. 等待 vbw-cli 退出
...
daemon 的 stdout/stderr 写入 `~/.vibewisp/logs/daemon-{timestamp}.log`，按日期时间命名。CLI 的 stdout/stderr 直通终端（用户交互）。
