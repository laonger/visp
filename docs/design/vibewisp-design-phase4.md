...
| Crate | 职责 | 类型 |
|---|---|---|
| **vbw-cli** | gRPC 客户端、REPL 交互、流式显示 | 新建 |
...
**依赖**：
- `vbw-proto`：gRPC 类型和 service client
- `tonic`：gRPC 客户端
- `tokio`：异步运行时
...
vbw-cli 只依赖 vbw-proto（通过 gRPC 协议与 daemon 通信），不依赖 vbw-core/vbw-llm/vbw-tools。
...
- `cargo build -p vbw-cli` 编译通过
- `cargo clippy -p vbw-cli -- -D warnings` 通过
- `cargo fmt --check` 通过
