# vibewisp Phase 5 阶段设计：CodeGraph 代码智能

## 1. 阶段目标

实现基于 tree-sitter 的代码解析、索引和查询引擎，为 vibewisp 提供代码智能能力（符号搜索、调用关系查询）。

**一句话总结**：对 TypeScript/JavaScript 项目完成全量索引后，支持按名称搜索符号和查询符号的调用者/被调用者。

**独立性**：CodeGraph 是独立模块，与 Phase 3（Agent 核心）和 Phase 4（CLI 前端）可并行开发。最终由 vbw-daemon 导入，为已定义的 gRPC RPC（SearchSymbols、GetSymbolDetails）提供实现。

## 2. 模块划分

Phase 5 涉及一个新 crate（vbw-codegraph）和一个已有 crate 的扩展（vbw-daemon 集成）。

| Crate | 职责 | 类型 |
|---|---|---|
| **vbw-codegraph** | tree-sitter 解析、符号/关系提取、索引构建、查询引擎、SQLite 持久化、文件监听 | 新建 |
| **vbw-daemon** | 集成 CodeGraph，实现 SearchSymbols / GetSymbolDetails RPC | 扩展 |

### 2.1 vbw-codegraph crate
...
**设计决策**：
- 每个项目一个独立的 SQLite 数据库文件，路径为 `<project_path>/.vibewisp/codegraph.db`
- `open` 时若 `.vibewisp` 目录不存在则自动创建
...
Phase 3 中 vbw-daemon 的 `SearchSymbols` 和 `GetSymbolDetails` RPC 返回 `unimplemented`。Phase 5 提供真实实现。

**集成方式**：

- vbw-daemon 的 `Cargo.toml` 添加 `vbw-codegraph` 依赖
...
注：数据库文件固定为 `<project_path>/.vibewisp/codegraph.db`，无需配置。

## 3. 依赖关系

```
vbw-codegraph (新 crate)
    ├──→ tree-sitter (C 库 FFI)
...
- `cargo build --workspace` 编译通过（包含 vbw-codegraph）
