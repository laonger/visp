# vibewisp Phase 2 阶段设计：LLM Provider + 内置工具
...
| Crate | 职责 | 类型 |
|---|---|---|
| **vbw-core** | 扩展：LlmError 枚举 + LlmConfig 扩展字段 + trait 错误类型修正 | 修改 |
| **vbw-llm** | Anthropic provider 实现 + SSE 流解析 + mock provider | 新建 |
| **vbw-tools** | 文件读写、bash 执行、grep/glob 搜索 | 新建 |
...
写入策略（防进程崩溃导致文件损坏）：
1. 将新内容写入目标文件同目录下的临时文件（`{filename}.vbw-tmp`）
...
```
vbw-core (扩展 Error + trait)
    ↑
    ├── vbw-llm (实现 LlmProvider trait)
    │      依赖: reqwest, serde_json, tokio
    │
    └── vbw-tools (实现 Tool trait)
           依赖: tokio (bash), 无额外 crate（搜索用系统命令）
```

vbw-llm 和 vbw-tools 互不依赖，可并行开发。
