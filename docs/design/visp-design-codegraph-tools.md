...
工具从 `ToolContext.working_dir` 获取项目路径，直接打开 `.visp/codegraph.db` 查询。
...
CodeGraphSearch::execute(args, context: ToolContext)
  ...
  │
  ├─ db_path = context.working_dir / ".visp" / "codegraph.db"
...
│  │       │                                                   │   │
│  │       ▼                                                   │   │
│  │  .visp/codegraph.db  (SQLite WAL + FTS5)              │   │
│  │       │                                                   │   │
│  │       ├─ search(query, limit)                              │   │
│  │       └─ get_details(name)                                 │   │
│  └──────────────────────────────────────────────────────────┘   │
...
```rust
// crates/visp-tools/src/codegraph.rs（简化）
async fn execute(&self, args: Value, ctx: &ToolContext) -> ToolResult {
    let query = args["query"].as_str()?;
    let limit = args["limit"].as_i64().unwrap_or(20) as usize;

    let db = ctx.working_dir.join(".visp").join("codegraph.db");
    if !db.exists() {
...
| 文件 | 改动 |
|------|------|
| `visp-tools/Cargo.toml` | 新增 `visp-codegraph` 依赖 |
| `visp-tools/src/codegraph.rs` | **新增**：`CodeGraphSearch` + `CodeGraphGetDetails` |
| `visp-tools/src/lib.rs` | 新增 `pub mod codegraph` |
| `visp-daemon/src/main.rs` | 注册两个工具 |
...
| SQLite 查询 | 沿用 `visp-codegraph` 已有测试 |
