...
```
vbw-cli ──────► vbw-proto ──────► vbw-core
                     ▲                   │
vbw-daemon ──────────┘      ┌────────────┼────────────────┐
                            ▼            ▼                ▼
                        vbw-llm    vbw-tools    vbw-codegraph
                                                      │
                                                 vbw-mcp
```

- **vbw-core**：纯逻辑 crate，无 IO 依赖。定义核心 trait 和类型。
- **vbw-proto**：gRPC 协议定义，生成的客户端/服务端代码。
- **vbw-llm**：LLM 提供商抽象和实现。
- **vbw-tools**：内置工具的具体实现。
- **vbw-codegraph**：代码智能引擎，依赖 tree-sitter。
- **vbw-mcp**：MCP 协议客户端实现。
- **vbw-daemon**：daemon 二进制，组装所有模块。
- **vbw-cli**：CLI 二进制，gRPC 客户端 + TUI。
...
- 规则文件内容（来自 `~/.config/vibewisp/rules/` 等路径）
...
1. 命令行参数（会话级）
2. 环境变量
3. 项目配置（`.vibewisp/config.toml`）
4. 全局配置（`~/.config/vibewisp/config.toml`）
5. 内置默认值
...
| 维度 | 现有 OpenCode (Node.js) | vibewisp (Rust) |
|---|---|---|
