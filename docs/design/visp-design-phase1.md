...
| Crate | 职责 | 依赖 |
|---|---|---|
| **visp-core** | 核心 trait、错误类型、消息类型 | 无（纯逻辑） |
| **visp-proto** | gRPC 协议定义，prost/tonic 代码生成 | 无 |

### 2.1 visp-core

这是整个项目的基石。所有其他 crate 通过实现 visp-core 的 trait 来接入系统。Phase 1 只定义接口，不实现。
...
**visp-llm 定义（Phase 2 实现，Phase 1 预留类型占位）**：
- **LlmError**：网络错误、速率限制、API 错误、流错误

**visp-tools 定义（Phase 2 实现，Phase 1 预留类型占位）**：
...
定义 visp daemon 与前端之间通信的 gRPC 协议。

#### 2.2.1 服务定义

**CoderDaemon 服务**（proto package: `visp`）提供以下 RPC 方法：
...
- 服务名在 proto 文件中定义为 `visp.CoderDaemon`
...
└── docs/
    ├── design/
    │   ├── visp-design.md      # ✅ 总设计文档
    │   └── visp-design-phase1.md # ✅ 当前文件
    └── plans/
        ├── visp-master-plan.md  # ✅ 总计划
        └── visp-plan-phase1.md  # Phase 1 计划（下一步）
```
...
visp-core (无外部依赖)
    ↑ 依赖
visp-proto (仅依赖 prost/tonic，不依赖 visp-core)
```

Phase 1 的两个 crate 相互独立。visp-proto 不依赖 visp-core，因为 protobuf 生成的类型是独立的数据结构，上层使用时再做类型转换。
...
- ❌ 不创建 `visp-llm`、`visp-tools`、`visp-daemon` 等 crate 目录（它们只需要在 workspace Cargo.toml 中声明为 member）
...
- visp-core 暴露的所有 public 类型和 trait 已在文档注释中说明用途
