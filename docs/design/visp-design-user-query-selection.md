...
| 层次 | 改动内容 |
|------|---------|
| **visp-proto** | UserQuery 增加 `options` / `allow_other`；UserResponse 改为 `selected_index` + `text` |
| **visp-core** | AgentEvent::UserQuery 的 respond 从 `Sender<bool>` 改为支持索引和文本；新增 Always Allow 机制 |
