...
为 visp 添加 MCP（Model Context Protocol）支持，使 daemon 能够连接外部 MCP 服务器，将其提供的工具动态集成到 Tool Registry 中，供 Agent 编排器调用。
...
| Crate | 职责 | 类型 |
|---|---|---|
| **visp-mcp** | MCP 客户端封装、工具适配器、服务器进程管理 | 新建 |
| **visp-daemon** | 配置加载 MCP 服务器列表、启动连接、注册工具到 Registry | 扩展 |
...
- `visp-core`：项目内，依赖 `Tool` trait 和 `ToolResult`
...
在 `visp-daemon/src/main.rs` 中，新增 MCP 初始化步骤（在工具注册之后）：
...
**变更**：只影响 `visp-core/src/tool_registry.rs`，Agent 循环和 daemon 的调用代码不变。
...
- `cargo test --workspace` 所有测试通过（含 visp-mcp 新增测试）
