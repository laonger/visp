...
| 组件 | 改动 |
|---|---|
| `visp.proto` | 新增 `Cancel` 消息 + `ClientMessage` oneof 新变体 |
| `visp-daemon service.rs` | Chat handler 中每轮 UserInput 独立 spawn agent task 和转发 task；新增 Cancel 消息处理 |
| `visp-proto` | 重新编译生成 Rust 代码（自动） |
