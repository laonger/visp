# vibewisp Phase 2 工作计划：LLM Provider + 内置工具

## 概述

Phase 2 实现 LLM 调用和基础工具。先扩展 vbw-core 的类型和 trait，再分 5 个 Wave 并行构建 vbw-llm 和 vbw-tools。
...
运行 `cargo test -p vbw-core` 确认失败（类型尚不存在）。
...
git commit -m "feat(vbw-core): add LlmError enum with five variants"
```
...
在 `crates/vbw-core/src/provider.rs` 的测试模块中编写：
...
```bash
git add crates/vbw-core/src/provider.rs
git commit -m "feat(vbw-core): extend LlmConfig with extra HashMap field"
```
...
```bash
git add crates/vbw-core/src/provider.rs
git commit -m "refactor(vbw-core): replace String errors with LlmError in LlmProvider trait"
```
...
在 `crates/vbw-core/src/tool.rs` 的测试模块中编写：
...
git commit -m "feat(vbw-core): add session_id field to ToolContext"
```

---

## 步骤 2：创建 vbw-llm crate（委托 @fixer）
...
创建 crate 结构，无需测试。验证：`cargo build -p vbw-llm` 失败（crate 尚不存在）。

#### 🟢 绿 — 实现

- 创建 `crates/vbw-llm/Cargo.toml`
  - 依赖：vbw-core, reqwest, serde, serde_json, tokio, futures, async-trait
- 创建 `crates/vbw-llm/src/lib.rs`（空模块声明）

#### 🧪 测试

```bash
cargo build -p vbw-llm
```
...
```bash
cargo clippy -p vbw-llm -- -D warnings
```
...
```bash
git add crates/vbw-llm/ Cargo.toml Cargo.lock
git commit -m "feat(vbw-llm): create crate skeleton"
```
...
在 `crates/vbw-llm/src/streaming.rs` 的测试模块中编写：
...
```bash
git add crates/vbw-llm/src/streaming.rs
git commit -m "feat(vbw-llm): SSE stream parser for Anthropic events"
```
...
创建 `crates/vbw-llm/src/anthropic.rs` 测试模块，编写：
...
```bash
git add crates/vbw-llm/src/anthropic.rs
git commit -m "feat(vbw-llm): Anthropic request builder with message merging"
```
...
```bash
git add crates/vbw-llm/src/anthropic.rs
git commit -m "feat(vbw-llm): Anthropic event parser and streaming chat implementation"
```
...
创建 `crates/vbw-llm/src/mock.rs` 测试模块，编写：
...
```bash
git add crates/vbw-llm/src/mock.rs crates/vbw-llm/src/lib.rs
git commit -m "feat(vbw-llm): add MockProvider for testing"
```

---

## 步骤 3：创建 vbw-tools crate（委托 @fixer）
...
验证 `cargo build -p vbw-tools` 失败（crate 尚不存在）。

#### 🟢 绿 — 实现

- 创建 `crates/vbw-tools/Cargo.toml`
  - 依赖：vbw-core, tokio, async-trait
- 创建 `crates/vbw-tools/src/lib.rs`（空模块声明）

#### 🧪 测试

```bash
cargo build -p vbw-tools
```

#### 🔍 类型检查

```bash
cargo clippy -p vbw-tools -- -D warnings
```
...
```bash
git add crates/vbw-tools/ Cargo.toml Cargo.lock
git commit -m "feat(vbw-tools): create crate skeleton"
```
...
创建 `crates/vbw-tools/src/path.rs` 测试模块，编写：
...
```bash
git add crates/vbw-tools/src/path.rs crates/vbw-tools/src/lib.rs
git commit -m "feat(vbw-tools): path safety validation with symlink resolution"
```
...
创建 `crates/vbw-tools/src/truncate.rs` 测试模块，编写：
...
```bash
git add crates/vbw-tools/src/truncate.rs crates/vbw-tools/src/lib.rs
git commit -m "feat(vbw-tools): output truncation with configurable limit"
```
...
创建 `crates/vbw-tools/src/file.rs` 测试模块，编写：
...
```bash
git add crates/vbw-tools/src/file.rs crates/vbw-tools/src/lib.rs
git commit -m "feat(vbw-tools): ReadFile tool with path safety, size limit, and binary detection"
```
...
```bash
git add crates/vbw-tools/src/file.rs
git commit -m "feat(vbw-tools): WriteFile tool with auto parent dir creation"
```
...
- execute 流程：
  ...
  4. 0 次 → 返回错误
  5. >1 次 → 返回错误 + 所有行号
  6. 1 次 → 先写临时文件 `.vbw-tmp`，再 `rename` 到目标文件（原子写入）
...
```bash
git add crates/vbw-tools/src/file.rs
git commit -m "feat(vbw-tools): EditFile tool with unique match constraint and atomic write"
```
...
创建 `crates/vbw-tools/src/bash.rs` 测试模块，编写：
...
```bash
git add crates/vbw-tools/src/bash.rs crates/vbw-tools/src/lib.rs
git commit -m "feat(vbw-tools): Bash tool with safety blacklist, timeout, and UTF-8 safety"
```
...
创建 `crates/vbw-tools/src/search.rs` 测试模块，编写：
...
```bash
git add crates/vbw-tools/src/search.rs crates/vbw-tools/src/lib.rs
git commit -m "feat(vbw-tools): Grep tool with rg priority, platform grep fallback, timeout and binary skip"
```
...
```bash
git add crates/vbw-tools/src/search.rs
git commit -m "feat(vbw-tools): Glob tool with rg/files priority and find fallback"
```
...
## Wave 1：vbw-core 扩展（1 个 Agent，串行）

```
Agent A: 1a → 1b → 1c → 1d
```

4 个子步骤串行执行。vbw-core 是基础设施，所有后续 crate 依赖它。
...
```
Agent A: 2a (vbw-llm 骨架)
Agent B: 3a (vbw-tools 骨架)
```
...
|---|---|---|---|---|
| 1 | 1 | vbw-core | 1a~1d (4) | 8 |
| 2 | 2 | vbw-llm + vbw-tools | 2a, 3a | 0 (骨架) |
| 3 | 5 | vbw-llm + vbw-tools | 2b, 2c, 2e, 3b, 3c | 13 |
| 4 | 4 | vbw-llm + vbw-tools | 2d, 3d~3i | 28 |
| 5 | — | 全 workspace | 质量门 | — |
...
- vbw-tools 不依赖任何额外 crate（搜索用系统命令，其他用标准库）
