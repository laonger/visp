...
## 步骤 1：visp-proto — 协议定义

### 1a：更新 UserQuery proto

#### 🔴 红 — 测试

proto 生成的 Rust 类型有新字段即可验证，通过 `cargo build -p visp-proto` 检查。
...
- 重新生成 Rust 代码（`cargo build -p visp-proto`）
...
在 `crates/visp-core/src/agent.rs` 现有测试文件追加：
...
在 `crates/visp-core/src/session.rs` 现有测试文件追加：
...
在 `crates/visp-core/src/agent.rs` 现有 `test_user_query_approved` / `test_user_query_denied` 测试基础上更新：
...
在 `crates/visp-core/src/prompt.rs` 或系统 prompt 模板中，增加一段关于 `[USER_QUERY]` 格式的指令：
...
如果 system prompt 模板使用内置默认字符串，直接在常量中追加；如果是可加载的外部模板（`.visp/system-prompt.md`），则需要在项目中更新默认模板。
...
```bash
cargo test -p visp-daemon && cargo test -p visp-core
cargo clippy -p visp-daemon -- -D warnings && cargo clippy -p visp-core -- -D warnings
```
