# vibewisp 工作计划：BlockStyle 统一渲染重构

## 概述

基于 `vibewisp-design-block-abstraction.md`，将 `render_chat_area` 中的分散 if-else 渲染逻辑替换为统一的 `BlockStyle` + `render_block()` 抽象，删除 `build_text_stack`。
...
#### 📦 提交
```
refactor(vbw-cli): add BlockStyle struct with four type presets
```
...
#### 📦 提交
```
refactor(vbw-cli): add viewport_intersect and unified render_block function
```
...
#### 📦 提交
```
refactor(vbw-cli): rewrite render_chat_area with unified BlockStyle loop
```
...
#### 📦 提交
```
refactor(vbw-cli): remove build_text_stack, replaced by ensure_all_caches
```
...
#### 📦 提交
```
refactor(vbw-cli): remove streaming_rendered_len/lines from AppState
```
...
#### 📦 提交
```
test(vbw-cli): verify BlockStyle refactor passes all tests
```
