...
本文档定义 vibewisp 项目从零到 MVP 的总体执行路线。每个阶段（Phase）在进入前会有独立的阶段性设计文档、工作计划、以及需求反思讨论。
...
项目骨架            LLM + 工具         Agent + Daemon     CLI 前端           CodeGraph
  │                  │                   │                  │                  │
  │── vbw-core       │── vbw-llm         │── Rule Engine    │── vbw-cli        │── 解析器
  │── vbw-proto      │── vbw-tools       │── Session Mgr    │── 流式输出       │── 索引
  │                  │                   │── Agent 循环     │── REPL           │── 查询
  │                  │                   │── vbw-daemon ────│                  │── 持久化
  │                  │                   │                  │                  │── 文件监听
  ▼                  ▼                   ▼                  ▼                  ▼
...
- Phase 2 依赖 Phase 1（需要 vbw-core 的 trait 定义）
...
**交付物**：
- `vibewisp/` 目录结构，workspace 级别的 Cargo.toml
- `vbw-core` crate：Tool trait、LlmProvider trait、Error 类型、Message 类型
- `vbw-proto` crate：vibewisp.proto 定义，tonic/prost 编译配置
...
**交付物**：
- `vbw-llm` crate：OpenAI provider、Anthropic provider、SSE 流解析
- `vbw-tools` crate：文件读写工具、bash 执行工具、grep/glob 搜索工具
...
**交付物**：
- `vbw-core` 扩展：Rule Engine（规则文件加载/热重载）、Session Manager（会话生命周期）、Prompt Builder、Agent 编排循环
- `vbw-daemon` crate：gRPC server 启动、配置加载、Service 实现（CreateSession/Chat/HealthCheck）
...
**目标**：实现终端交互界面，用户可通过 `vbw` 命令使用 vibewisp。

**交付物**：
- `vbw-cli` crate：gRPC 客户端、流式输出显示、基础 REPL 模式
...
**交付物**：
- `vbw-codegraph` crate：tree-sitter 解析器封装、符号提取、关系提取、符号图索引、查询引擎
...
docs/
├── design/
│   ├── vibewisp-design.md              # ✅ 总设计文档 (已完成)
│   ├── vibewisp-design-phase1.md       # Phase 1 阶段设计
│   ├── vibewisp-design-phase2.md       # Phase 2 阶段设计
│   ├── ...
│
└── plans/
    ├── vibewisp-master-plan.md         # ✅ 总计划文档 (当前文件)
    ├── vibewisp-plan-phase1.md         # Phase 1 阶段计划
    ├── vibewisp-plan-phase2.md         # Phase 2 阶段计划
    └── ...
```
