# vibewisp 工作计划：/init 斜杠命令

## 概述

实现 `/init` 斜杠命令，一键完成项目初始化：创建 `.vibewisp/` 目录、初始化 CodeGraph、构造 prompt 并启动 agent loop 生成 AGENTS.md。

涉及文件：`cli/event.rs`、`daemon/src/command/init.rs`（新增）、`daemon/src/command/mod.rs`（新增）、`daemon/src/service.rs`、`vbw-core/src/message.rs`、`vbw-core/src/prompt.rs`。
...
```bash
cargo test -p vbw-core && cargo clippy -- -D warnings
```
...
```bash
cargo test -p vbw-cli && cargo clippy -- -D warnings
```
...
| 2 | `prepare()` 创建 .vibewisp/ 目录 | 验证 `.vibewisp/rules/`、`skills/`、`plans/` 目录被创建 |
| 3 | `.vibewisp/` 已存在时幂等 | 目录已存在时不报错，正常继续 |
| 4 | `prepare()` 创建 CodeGraph 数据库 | 验证 `.vibewisp/codegraph.db` 被创建 |
...
```bash
cargo test -p vbw-daemon && cargo clippy -- -D warnings
```
...
```
vbw-core/message.rs ──→ vbw-core/prompt.rs ──→ (其他步骤的基础)
                                                    │
cli/event.rs ────────────────────────────────────────┤
                                                     ├──→ daemon/service.rs
...
| Wave | 并行 | 模块 | 步骤 | 测试用例 |
|------|------|------|------|---------|
| 0 | 串行 | vbw-core/message.rs | 0a | 2 |
| 0 | 串行 | vbw-core/prompt.rs | 0b | 2 |
