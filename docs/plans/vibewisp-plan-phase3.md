# vibewisp Phase 3 工作计划：Agent 核心 + Daemon

## 概述

Phase 3 实现 Agent 编排循环和 gRPC daemon 服务。先扩展 vbw-core 的基础类型，再实现核心模块，最后构建 vbw-daemon。

每个子步骤都是一个独立的 TDD 循环：**红 → 绿 → 测试 → 类型检查 → 重构 → 提交**。

---

## 步骤 1：vbw-core 基础类型扩展

### 1a：SessionError 扩展 + AgentErrorCode 枚举

#### 🔴 红 — 测试

在 `crates/vbw-core/src/error.rs` 的测试模块中编写：
...
```bash
cargo test -p vbw-core && cargo clippy -p vbw-core -- -D warnings
```

#### 📦 提交

```bash
git add crates/vbw-core/src/error.rs
git commit -m "feat(vbw-core): add SessionBusy, ProtocolError to SessionError; add AgentErrorCode enum"
```
...
在 `crates/vbw-core/src/agent.rs` 新建测试模块：
...
- 新建 `crates/vbw-core/src/agent.rs`
...
```bash
git add crates/vbw-core/src/agent.rs crates/vbw-core/src/lib.rs
git commit -m "feat(vbw-core): add AgentConfig struct with defaults"
```
...
在 `crates/vbw-core/src/tool.rs` 测试模块中编写：
...
```bash
git add crates/vbw-core/src/tool.rs
git commit -m "feat(vbw-core): add requires_approval method to Tool trait"
```
...
在 `crates/vbw-core/src/agent.rs` 中编写：
...
```bash
git add crates/vbw-core/src/agent.rs
git commit -m "feat(vbw-core): add AgentEvent enum and AgentLoopContext struct"
```
...
新建 `crates/vbw-core/src/rules.rs`，编写测试：
...
- 扫描逻辑：递归 `project_path/.vibewisp/rules/` 和 `~/.config/vibewisp/rules/`，仅 `.md` 文件，前 5 行内查找 `alwaysApply: true`
...
```bash
git add crates/vbw-core/src/rules.rs crates/vbw-core/Cargo.toml
git commit -m "feat(vbw-core): RuleEngine with file loading and notify hot reload"
```
...
新建 `crates/vbw-core/src/session.rs`，编写测试：
...
```bash
git add crates/vbw-core/src/session.rs crates/vbw-core/src/lib.rs
git commit -m "feat(vbw-core): SessionManager with state machine and CancellationToken management"
```
...
新建 `crates/vbw-core/src/prompt.rs`，编写测试：
...
```bash
git add crates/vbw-core/src/prompt.rs crates/vbw-core/src/lib.rs
git commit -m "feat(vbw-core): PromptBuilder for system message and history assembly"
```
...
新建 `crates/vbw-core/src/tool_registry.rs`，编写测试：
...
```bash
git add crates/vbw-core/src/tool_registry.rs crates/vbw-core/src/lib.rs
git commit -m "feat(vbw-core): ToolRegistry for tool registration, lookup, and execution"
```
...
```bash
git add crates/vbw-core/src/agent.rs
git commit -m "feat(vbw-core): Agent orchestration loop with tool execution and human-in-loop"
```

---

## 步骤 4：vbw-core lib.rs 更新
...
```bash
git add crates/vbw-core/src/lib.rs
git commit -m "feat(vbw-core): update lib.rs module declarations and re-exports"
```
...
修改 `crates/vbw-proto/proto/vibewisp.proto`：
...
- `cargo build -p vbw-proto` 编译通过，生成的 Rust 代码包含新消息
- `cargo clippy -p vbw-proto -- -D warnings` 通过

#### 📦 提交

```bash
git add crates/vbw-proto/proto/vibewisp.proto crates/vbw-proto/src/
git commit -m "feat(vbw-proto): add UserQuery/UserResponse for human-in-loop; refactor ConfigUpdate to typed fields"
```

---

## 步骤 6：vbw-daemon crate

### 6a：项目骨架

#### 🔴 红 — 验证

`cargo build -p vbw-daemon` 失败（crate 尚不存在）。

#### 🟢 绿 — 实现

- 创建 `crates/vbw-daemon/Cargo.toml`，依赖：vbw-core, vbw-proto, vbw-llm, vbw-tools, tonic, tokio, toml, tracing, tracing-subscriber
- 创建 `crates/vbw-daemon/src/main.rs`（最小入口）
- Workspace Cargo.toml 中 `members` 添加 `"crates/vbw-daemon"`

#### 🧪 测试 → 🔍 类型检查

```bash
cargo build -p vbw-daemon && cargo clippy -p vbw-daemon -- -D warnings
```

#### 📦 提交

```bash
git add crates/vbw-daemon/ Cargo.toml Cargo.lock
git commit -m "feat(vbw-daemon): create crate skeleton"
```
...
```bash
git add crates/vbw-daemon/src/config.rs
git commit -m "feat(vbw-daemon): TOML config loading with env var override"
```
...
```bash
git add crates/vbw-daemon/src/server.rs
git commit -m "feat(vbw-daemon): gRPC server startup with graceful shutdown"
```
...
```bash
git add crates/vbw-daemon/src/service.rs crates/vbw-daemon/tests/
git commit -m "feat(vbw-daemon): CoderDaemon service with Chat, Session, and HealthCheck"
```
...
```bash
git add crates/vbw-daemon/src/main.rs crates/vbw-daemon/Cargo.toml
git commit -m "feat(vbw-daemon): main entry with startup orchestration"
```
...
### Wave 1：vbw-core 基础类型（1 个 Agent，串行）

```
Agent A: 1a → 1b → 1c → 1d
```

4 个子步骤串行执行。vbw-core 的基础类型是后续所有模块的依赖。

### Wave 2：vbw-core 核心模块（4 个 Agent，并行）
...
### Wave 4：vbw-proto + vbw-daemon 骨架（2 个 Agent，并行）

```
Agent A: 步骤 5 (vbw-proto)
Agent B: 6a (vbw-daemon 骨架)
```

### Wave 5：vbw-daemon 模块（3 个 Agent，并行）
...
|---|---|---|---|---|
| 1 | 1 | vbw-core | 1a~1d (4) | 10 |
| 2 | 4 | vbw-core | 2a~2d (4) | 23 |
| 3 | 2 | vbw-core | 3~4 (2) | 9 |
| 4 | 2 | vbw-proto + vbw-daemon | 5+6a (2) | 0 |
| 5 | 3 | vbw-daemon | 6b~6e (3) | 9 |
| 6 | — | 全 workspace | 质量门 | — |
...
- Agent 循环测试使用 `MockProvider`（vbw-llm 已提供），不依赖真实 API
- vbw-core 新增依赖：`tokio`（mpsc + CancellationToken）、`notify`（Rule Engine 热重载）
- vbw-daemon 新增依赖：`toml`（配置解析）、`tonic`（gRPC）
