...
在 `crates/vbw-cli/src/app.rs` 中：

- `AppState` 增加 `pub stale_done_expected: bool` 字段
- 在 `AppState::new()` 中初始化 `stale_done_expected: false`

在 `crates/vbw-cli/src/event.rs` 中修改 **Esc handler**（`else` 分支，非 Other 模式）：
...
在 `crates/vbw-core/src/agent.rs` 中，将 LLM 事件收集循环（约 384-422 行）从 `while let` 改为 `tokio::select!`：
...
在 `crates/vbw-core/src/agent.rs` 中，修改约 670-671 行：
...
在 `crates/vbw-daemon/src/service.rs` 中：
