...
涉及：vbw-core（Tool trait 扩展）、vbw-tools（新工具）、vbw-daemon（配置+注册）。
...
**涉及文件**：`crates/vbw-core/src/tool.rs`
...
`feat(vbw-core): add requires_approval_for to Tool trait for argument-aware approval`
...
`crates/vbw-core/src/agent.rs` 中，将：
...
`feat(vbw-core): use requires_approval_for in agent loop with argument display`
...
**涉及文件**：`crates/vbw-daemon/src/config.rs`
...
`feat(vbw-daemon): add generic tool config container to DaemonConfig`
...
| 26 | `test_load_project_config_exists` | `.vibewisp/webfetch.toml` 存在 → 返回 domains |
| 27 | `test_load_project_config_not_exists` | 文件不存在 → 空列表 |

#### 🟢 绿 — 实现

`crates/vbw-tools/src/fetch.rs`，包含：
...
```bash
cargo test -p vbw-tools && cargo clippy -p vbw-tools -- -D warnings
```

#### 📦 提交

`feat(vbw-tools): implement WebFetch tool with URL validation, whitelist, and HTML-to-Markdown`
...
`feat(vbw-daemon): register WebFetch tool from daemon config`
...
|------|------|-------|------|---------|
| 1 | 1 | vbw-core | 1b, 1c | 2 |
| 2 | 2 | vbw-daemon, vbw-tools | 2a, 3a | 27+3 |
| 3 | 1 | vbw-tools, vbw-daemon | 4a, 4b | 0（编译验证） |
| 4 | — | 全 workspace | 5 | — |
...
- 项目级白名单文件 `.vibewisp/webfetch.toml` 不存在时静默跳过
