...
优化默认 system prompt 的内容密度和结构清晰度，让 LLM 清楚自己的角色、编码规范、可用工具和交互方式。同时改进工具描述和参数文档，实现动态工具指南。仅涉及 visp-core 和 visp-daemon 两个 crate。
...
Wave 1 和 Wave 2 修改的是 visp-core 中不同的文件集合，但 agent.rs 被两个 Wave 都修改（不同位置），所以串行执行。
...
| # | 测试用例 | 说明 |
|---|---------|------|
| 1.1 | 默认 prompt 包含角色定义 | 含 "visp" 和 "Rust" |
...
在 `crates/visp-core/src/session.rs` 中，用 `concat!()` 宏重写 `DEFAULT_SYSTEM_PROMPT`：
...
在 `crates/visp-core/src/agent.rs` 中：
...
更新 `crates/visp-core/src/prompt.rs` 中的测试：
...
在 `crates/visp-core/src/tool.rs` 的 `Tool` trait 中增加：
...
```bash
cargo test -p visp-core -p visp-tools
cargo clippy -p visp-core -p visp-tools -- -D warnings
```
...
在 `crates/visp-daemon/src/main.rs` 中调整 register 调用顺序：
