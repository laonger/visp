# visp Phase 5 工作计划：CodeGraph 代码智能

## 概述

Phase 5 实现 tree-sitter 代码解析引擎和 SQLite 索引查询系统。创建新 crate visp-codegraph，扩展 visp-daemon。

每个子步骤都是一个独立的 TDD 循环：**红 → 绿 → 测试 → 类型检查 → 重构 → 提交**。

---

## 步骤 1：visp-codegraph 项目骨架 + 依赖

### 🔴 红 — 验证

`cargo build -p visp-codegraph` 失败（crate 尚不存在）。

### 🟢 绿 — 实现

- 创建 `crates/visp-codegraph/Cargo.toml`，依赖：`visp-core`, `tree-sitter`, `tree-sitter-typescript`, `rusqlite` (feature: `bundled`), `walkdir`, `notify`, `tokio`
- 创建 `crates/visp-codegraph/src/lib.rs`（空模块声明）
- Workspace Cargo.toml 中 `members` 添加 `"crates/visp-codegraph"`
- Workspace Cargo.toml 中添加 `tree-sitter`、`tree-sitter-typescript`、`rusqlite`、`walkdir` 的 `[workspace.dependencies]`

### 🧪 测试 → 🔍 类型检查

```bash
cargo build -p visp-codegraph && cargo clippy -p visp-codegraph -- -D warnings
```

### 📦 提交

```bash
git add crates/visp-codegraph/ Cargo.toml Cargo.lock
git commit -m "feat(visp-codegraph): create crate skeleton with dependencies"
```
...
```bash
git add crates/visp-codegraph/src/graph.rs
git commit -m "feat(visp-codegraph): Symbol and SymbolKind types"
```
...
```bash
git add crates/visp-codegraph/src/graph.rs
git commit -m "feat(visp-codegraph): Edge and EdgeKind types with unresolved edge support"
```
...
```bash
git add crates/visp-codegraph/src/graph.rs
git commit -m "feat(visp-codegraph): FileInfo type for indexed file metadata"
```
...
```bash
git add crates/visp-codegraph/src/store.rs crates/visp-codegraph/src/lib.rs
git commit -m "feat(visp-codegraph): Store with SQLite schema, WAL mode, and foreign keys"
```
...
```bash
cargo test -p visp-codegraph && cargo clippy -p visp-codegraph -- -D warnings
```

#### 📦 提交

```bash
git add crates/visp-codegraph/src/store.rs
git commit -m "feat(visp-codegraph): Store CRUD operations for symbols, edges, imports, exports"
```
...
```bash
git add crates/visp-codegraph/src/parser.rs
git commit -m "feat(visp-codegraph): tree-sitter parser for TypeScript symbol and edge extraction"
```
...
```bash
git add crates/visp-codegraph/src/index.rs crates/visp-codegraph/src/lib.rs
git commit -m "feat(visp-codegraph): full index build with cross-file resolution"
```
...
```bash
git add crates/visp-codegraph/src/index.rs
git commit -m "feat(visp-codegraph): incremental index update for file create/modify/delete"
```
...
```bash
git add crates/visp-codegraph/src/query.rs crates/visp-codegraph/src/lib.rs
git commit -m "feat(visp-codegraph): query engine with prefix search and symbol details"
```
...
```bash
git add crates/visp-codegraph/src/watcher.rs crates/visp-codegraph/src/lib.rs
git commit -m "feat(visp-codegraph): file watcher with debounce and incremental index triggers"
```
...
```bash
git add crates/visp-codegraph/src/lib.rs
git commit -m "feat(visp-codegraph): CodeGraph main struct with lifecycle management"
```
...
- visp-daemon Cargo.toml 添加 visp-codegraph 依赖
...
```bash
git add crates/visp-daemon/
git commit -m "feat(visp-daemon): CodeGraph integration for SearchSymbols and GetSymbolDetails"
```
...
### Wave 5：visp-daemon 集成（1 个 Agent）
...
|---|---|---|---|---|
| 1 | 1 | visp-codegraph | 步骤 1~2c (4) | 5 |
| 2 | 2 | visp-codegraph | 步骤 3~4 (3) | 26 |
| 3 | 2 | visp-codegraph | 步骤 5~6 (3) | 11 |
| 4 | 2 | visp-codegraph | 步骤 7~8 (2) | 9 |
| 5 | 1 | visp-daemon | 步骤 9 (1) | 3 |
| 6 | — | 全 workspace | 质量门 | — |
...
- visp-codegraph 不依赖 visp-llm 或 visp-tools，可完全独立开发
