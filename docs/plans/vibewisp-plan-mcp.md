...
基于设计文档 `vibewisp-design-mcp.md`，将 MCP 支持拆分为 6 个 Wave、11 个子步骤。

**影响范围**：vbw-core（修改）、vbw-mcp（新建）、vbw-daemon（扩展）
...
任务 B: 2a vbw-mcp crate + config + error
```
*Wave 1 无依赖，可完全并行*
...
Wave 1A (ToolRegistry) ─────────────────────────────┐
                                                     │
Wave 1B (vbw-mcp crate) ─→ Wave 2 (transport+session) ─→ Wave 3 (call+shutdown)
                                                           │
                                                           ├→ Wave 4 (ToolAdapter)
                                                           │       │
                                                           │       └→ Wave 5 (Manager)
...
2. **模块定位**：`vbw-mcp` 走依赖 vbw-core 的 `Tool` trait 和 `ToolResult`，不依赖 ToolRegistry 自身（回调注入）
