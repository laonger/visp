# vibewisp 工作计划：对话区渲染缓存

## 概述

基于 `vibewisp-design-render-cache.md` 设计文档，实现按消息粒度的渲染缓存系统，为后续 Markdown 解析和代码高亮预留扩展点。
...
#### 📦 提交
```
feat(vbw-cli): add id/version to ChatLine, auto-assign in add_message
```
...
#### 📦 提交
```
feat(vbw-cli): add update_message and clear_messages methods
```
...
#### 📦 提交
```
feat(vbw-cli): add MessageCache struct with per-message rendering
```
...
#### 📦 提交
```
feat(vbw-cli): add type-aware rendering and cache matching to MessageCache
```
...
#### 📦 提交
```
feat(vbw-cli): maintain message_caches in add_message
```
...
#### 📦 提交
```
feat(vbw-cli): add frozen_cache for long streaming text
```
...
#### 📦 提交
```
feat(vbw-cli): rewrite build_text_stack to assemble from message_caches
```
...
#### 📦 提交
```
feat(vbw-cli): invalidate message caches on terminal resize
```
...
#### 📦 提交
```
refactor(vbw-cli): use clear_messages() for /clear command
```
...
#### 🧪 测试 → 🔍 类型检查 → 格式检查
```bash
cargo test -p vbw-cli && cargo clippy -p vbw-cli -- -D warnings && cargo fmt -p vbw-cli -- --check
```

#### 📦 提交
```
test(vbw-cli): verify all tests pass after render cache refactor
```
