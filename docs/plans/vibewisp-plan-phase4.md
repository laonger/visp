# vibewisp Phase 4 工作计划：CLI 前端

## 概述

Phase 4 实现终端交互界面。创建新 crate vbw-cli，通过 gRPC 连接 daemon，提供 REPL 对话体验。
...
`cargo build -p vbw-cli` 失败（crate 不存在）。

### 🟢 绿 — 实现

- 创建 `crates/vbw-cli/Cargo.toml`，依赖：`vbw-proto`, `tonic`, `tokio` (rt-multi-thread + macros + signal), `clap` (derive), `rustyline` (default-features = false, features = ["with-file-history"])
- 创建 `crates/vbw-cli/src/main.rs`（最小入口：`fn main() {}`）
- 修改 workspace `Cargo.toml` 的 `members` 添加 `"crates/vbw-cli"`

### 🧪 测试 → 🔍 类型检查

```bash
cargo build -p vbw-cli && cargo clippy -p vbw-cli -- -D warnings
```

### 📦 提交

```bash
git add crates/vbw-cli/ Cargo.toml Cargo.lock && git commit -m "feat(vbw-cli): create crate skeleton"
```
...
新建 `crates/vbw-cli/src/client.rs`。在 main.rs 中添加 `mod client;`。
...
```bash
git add crates/vbw-cli/ && git commit -m "feat(vbw-cli): gRPC client with ChatHandle bidirectional stream"
```

### 2b：终端显示（display.rs）

新建 `crates/vbw-cli/src/display.rs`。在 main.rs 中添加 `mod display;`。
...
```bash
git add crates/vbw-cli/ && git commit -m "feat(vbw-cli): terminal display formatting with streaming text and truncation"
```
...
新建 `crates/vbw-cli/src/repl.rs`。在 main.rs 中添加 `mod repl;`。
...
```bash
git add crates/vbw-cli/ && git commit -m "feat(vbw-cli): REPL loop with select!, InputMode, and special commands"
```
...
```bash
git add crates/vbw-cli/ && git commit -m "feat(vbw-cli): main entry with CLI args, connection, and REPL startup"
```
...
|---|---|---|---|---|
| 1 | 1 | vbw-cli | 骨架 | 0 |
| 2 | 2 | vbw-cli | client + display | 3 |
| 3 | 2 | vbw-cli | repl + main | 0 |
| 4 | — | 全 workspace | 质量门 | — |
