# visp Phase 1 工作计划：项目骨架 + 核心抽象

## 概述

Phase 1 创建项目骨架，定义 `visp-core` 的核心 trait 和 `visp-proto` 的 gRPC 协议。
...
**操作**：
- 创建 `visp/Cargo.toml`（workspace 根，声明 8 个 member）
- 创建 \`visp/rust-toolchain.toml\`（channel = "stable"）
- 创建 `visp/crates/` 目录
- 创建 `visp/visp-core/Cargo.toml`（依赖：serde, serde_json, async-trait, thiserror, uuid）
- 创建 `visp/visp-core/src/lib.rs`（空模块声明）
- 创建 `visp/visp-proto/Cargo.toml`（依赖：tonic, prost, prost-types）
- 创建 `visp/visp-proto/build.rs`（prost 编译配置）
- 创建 `visp/visp-proto/src/lib.rs`（include! 生成代码）
...
**操作**：
- 创建 `visp/visp-proto/proto/visp.proto`
- 定义服务 `CoderDaemon` 和所有 RPC 方法
- 定义全部消息类型
- 需要 `google.protobuf.Timestamp` 依赖

**验证**：`cargo build -p visp-proto` 编译通过，proto 生成 Rust 代码无误

### 步骤 3：定义 visp-core 核心类型
...
**验证**：`cargo build -p visp-core` 编译通过
