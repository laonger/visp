...
验证：`cargo build -p vbw-proto` 通过，生成的 Rust 代码包含 `Cancel` 类型。

#### 📦 提交

```bash
git add crates/vbw-proto/ && git commit -m "feat(vbw-proto): add Cancel message for client-initiated agent cancellation"
```
...
修改 `crates/vbw-daemon/src/service.rs` 的 Chat handler。
...
```bash
git add crates/vbw-daemon/ && git commit -m "feat(vbw-daemon): handle Cancel message in Chat stream"
```
